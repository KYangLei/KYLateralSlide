//
//  KYSlideAnimatedTransitioning.swift
//  KYLateralSlideDemo
//
...
class KYSlideAnimatedTransitioning: NSObject,UIViewControllerAnimatedTransitioning {
    var type:KYSlideShowType!
    var config:KYLateralSlideConfig!
    
    init(showType: KYSlideShowType,config: KYLateralSlideConfig) {
        super.init()
        type = showType
        self.config = config
    }
    ...
    func animateTransitionShowType(transitionContext: UIViewControllerContextTransitioning) {
        ...
        var mask:KYSlideMaskView?
        for view in toController.view.subviews {
            if view.isKind(of: KYSlideMaskView.classForCoder()){
                mask = view as? KYSlideMaskView
                break
            }
        }
        if mask == nil {
            mask = KYSlideMaskView()
    ...
    func animateTransitionHiddenType(transitionContext: UIViewControllerContextTransitioning) {
        ...
        let flag: CGFloat! = config.direction == KYSlideDirection.left ? -1.0 : 1.0
        var fromTransform:CGAffineTransform = CGAffineTransform.init(translationX: flag * width, y: 0)
